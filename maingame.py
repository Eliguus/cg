...
while(1):
    ...
    sprite_sheet_image = pygame.image.load('pictures/New Piskel.png').convert_alpha()
    ...
    for i in range(no_animations):
	    list_animation.append(sprite_sheet.get_image(i, 64, 64, 3, BLACK))
    for i in range(no_animations):
	    list_animation2.append(sprite_sheet2.get_image(i, 64, 64, 3, BLACK))
    ...
    while (running):
        ...
        if current_time-last_update>animation_cool:
            ...
            if frame>=no_animations:
                frame=0
        screen.blit(list_animation[frame],(position,400))   

        screen.blit(list_animation2[frame-4],(position+2,550)) 
